...
module Channel_IO : sig
  ...
  val read :
    Reader.t -> [`Message of [`Error | `Output] * string | `Result of int] Def_error.t
end = struct
  let read_length child_stdout =
    ...
    >>| function
    | `Eof len ->
      Or_error.error "read_length: eof" (Bytes.To_string.sub ~pos:0 ~len buf) String.sexp_of_t
    | `Ok ->
      Ok (Binary_packing.unpack_unsigned_32_int_big_endian ~buf ~pos:0)
  ;;
  ...
  let read child_stdout =
    ...
    | `Ok channel_char ->
      ...
      | Ok channel ->
        ...
        >>| function
        | `Eof len ->
          Or_error.error "eof while reading message"
    ...
    let flatten outputs =
      let stdouts, stderrs =
        List.partition_map outputs ~f:(fun (channel, text) ->
          match channel with `Output -> First text | `Error -> Second text)
      in
    ...
    let rec loop acc =
      ...
      | `Result exit_code ->
        ...
        let exit_status =
          ...
          else Error (`Exit_non_zero exit_code)
        in
  ...
  let send_command child_stdin args =
    ...
    try_with ~run:(`Schedule) (* consider [~run:`Now] instead; see: https://wiki/x/ByVWF *) ~rest:(`Log) (* consider [`Raise] instead; see: https://wiki/x/Ux4xF *) (fun () ->
    ...
    | Ok _ as ok -> ok
    | Error exn ->
      Or_error.error "unable to write command; process is probably dead!"
        (args, exn) [%sexp_of: string list * exn]
  ;;

  let run_command process args =
    let child_stdin = Process.stdin process in
    send_command child_stdin args
    >>=? fun () ->
    read_full (Process.stdout process)
  ;;
end
...
let valid_hello ~accepted_encodings hello =
  ...
  Or_error.combine_errors_unit
    [ check "capabilities" ~f:(fun vals ->
        ...
        if List.exists capabilities ~f:is_runcommand then
          Ok ()
        else
          Or_error.error_s [%message
            "capabilities don't include runcommand"
              (capabilities : string list)
          ])
    ; check "encoding" ~f:(fun encoding ->
        if List.mem ~equal:String.equal accepted_encodings encoding then
          Ok ()
        else
          Or_error.error_s [%message
...
let%test _ =
  Result.is_ok
    (valid_hello ~accepted_encodings:[`Utf8]
       "capabilities: getencoding runcommand\nencoding: UTF-8")
let%test _ =
  Result.is_ok
    (valid_hello ~accepted_encodings:[`Ascii]
       "capabilities: getencoding runcommand\nencoding: ascii\n")
let%test _ =
  Result.is_error
    (valid_hello ~accepted_encodings:[`Utf8]
  ...
  let hello_result =
    ...
    | `Message (`Error, error) ->
      Deferred.Or_error.error_s
        [%message "replied on error channel" (error : string)]
    | `Result result ->
      Deferred.Or_error.error_s
        [%message "replied on result channel, expecting output channel" (result : int)]
    | `Message (`Output, text) ->
      return (valid_hello ~accepted_encodings text)
  in
